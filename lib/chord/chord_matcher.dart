...
class ChordMatcher {
  ...
  ChordFeedback evaluate({
    ...
    final weakThreshold = strongThreshold * weakRatio;

    final missingThreshold = strongThreshold * missingRatio;
    ...
    for (var stringNumber = 6; stringNumber >= 1; stringNumber--) {
      ...
      if (peakEnergy < missingThreshold ||

          absCents > wrongNoteCentsTolerance ||

          energy < missingThreshold) {

        status = NoteStatus.missing;

        message = _missingMessage(note);

      } else if (energy >= strongThreshold &&

          absCents <= perfectCentsTolerance) {
      ...
      } else if (energy >= weakThreshold &&

          absCents <= closeCentsTolerance) {
